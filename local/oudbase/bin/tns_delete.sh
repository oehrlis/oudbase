...
function Usage() {
  ...
  Common Options:
    ...
    -b <BASEDN>         Specify Base DN to delete Net Service Name. Either
                        specific base DN or ALL to delete Net Service Name from
    ...
    for basedn in ${BASEDN_LIST}; do 
            ...
            if net_service_exists "$current_cn" "${basedn}" ; then
                ...
                if ! dryrun_enabled; then
                    ldapdelete -p ${TVDLDAP_LDAPPORT} -h ${TVDLDAP_LDAPHOST} "$current_binddn" $current_bindpwd <<-EOI
